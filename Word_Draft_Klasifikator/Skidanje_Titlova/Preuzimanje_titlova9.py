...
def SkiniTitlove(title,id_filma):
    try:
        ...
        if pocetak != -1: #ako su titlovi nadeni
            tree = ET.parse('xml_titlovi9.xml')
            root = tree.getroot()
...
lista_filmova = open("Lista_Filmova9.txt", "r")    # UNIJETI NAZIV DATOTEKE S INFORMACIJAMA !!!!****!!!!
...
while i < len(film_list):
    ...
    if esi_mi_dobar:
         ...
         if skidanje_titlova =="fail":
            ...
            try:
                finalna_lista = open("Finalna_lista_filmova9.txt", "a") # IZLAZ !!! ****
